...
/** Dòng lẻ = tên bài, dòng chẵn = ca sĩ (lower third / OBS) */
function parseLowerThirdLines(linesText) {
  ...
  for (let i = 0; i < lines.length; i += 2) {
    const title = lines[i] || '';
    const singer = i + 1 < lines.length ? lines[i + 1] : '';
    tracks.push({ title, singer: singer || '—' });
  }
  return tracks;
}

function normalizeLowerThirdData(ld) {
  if (!ld || typeof ld !== 'object') return;
  if (!ld.linesText && (ld.title || ld.singer)) {
    const t = String(ld.title || '').trim();
    const s = String(ld.singer || '').trim();
    ld.linesText = [t, s].filter(Boolean).join('\n');
  }
  if (ld.linesText === undefined || ld.linesText === null) ld.linesText = '';
  if (typeof ld.activeIndex !== 'number' || Number.isNaN(ld.activeIndex)) {
    ld.activeIndex = 0;
  }
  const tracks = parseLowerThirdLines(ld.linesText);
  ld.tracks = tracks;
  if (tracks.length === 0) {
    ld.title = '';
    ld.singer = '';
  ...
  let idx = ld.activeIndex;
  if (idx < 0) idx = 0;
  if (idx >= tracks.length) idx = tracks.length - 1;
  ld.activeIndex = idx;
  ld.title = tracks[idx].title;
  ld.singer = tracks[idx].singer;
}
...
function readTenants() {
  try {
    ...
    Object.values(tenants).forEach(tenant => {
      ...
      normalizeLowerThirdData(tenant.lowerThirdData);
    });
...
app.post('/api/register', express.json(), async (req, res) => {
  ...
  tenants[tenantId] = {
    ...
    lowerThirdData: { linesText: '', title: '', singer: '', visible: false, activeIndex: 0, tracks: [] },
...
io.on('connection', (socket) => {
    ...
    if (tenantInfo) {
        ...
        if (!tenantInfo.tenant.lowerThirdData) {
          tenantInfo.tenant.lowerThirdData = {
            linesText: '',
            title: '',
            singer: '',
            visible: false,
        ...
        normalizeLowerThirdData(tenantInfo.tenant.lowerThirdData);
    ...
    socket.on('updateLowerThird', (data) => {
        ...
        if (data && data.linesText !== undefined) {
            ld.linesText = String(data.linesText);
        } else if (data && (data.title !== undefined || data.singer !== undefined)) {
            const t = data.title !== undefined ? String(data.title) : (ld.title || '');
            const s = data.singer !== undefined ? String(data.singer) : (ld.singer || '');
            ld.linesText = [t, s].map((x) => String(x).trim()).filter(Boolean).join('\n');
        }
        normalizeLowerThirdData(ld);
        emitToTenantRoom(tenantInfo.tenantId, 'lowerThirdUpdate', ld);
        writeTenants(tenants);
    });

    socket.on('toggleLowerThirdSlot', (payload) => {
        const tenantInfo = getTenantFromSocket();
        if (!tenantInfo) return;
        if (!tenantInfo.tenant.lowerThirdData) {
            tenantInfo.tenant.lowerThirdData = {
    ...
    socket.on('showLowerThird', () => {
        ...
        const ld = tenantInfo.tenant.lowerThirdData;
        normalizeLowerThirdData(ld);
        if (!ld.tracks || ld.tracks.length === 0) return;
        ld.visible = true;
        ld.title = ld.tracks[ld.activeIndex].title;
        ld.singer = ld.tracks[ld.activeIndex].singer;
        emitToTenantRoom(tenantInfo.tenantId, 'showLowerThird');
        emitToTenantRoom(tenantInfo.tenantId, 'lowerThirdUpdate', ld);
        writeTenants(tenants);
    });
    ...
    socket.on('join', (data) => {
        ...
        if (data.tenantId) {
            ...
            if (t && t.lowerThirdData) {
                normalizeLowerThirdData(t.lowerThirdData);
