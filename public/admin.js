...
const lowerThirdLines = document.getElementById('lowerThirdLines');
const lowerThirdSlotButtons = document.getElementById('lowerThirdSlotButtons');
let lastLowerThirdServerData = null;

function parseLowerThirdLinesClient(text) {
...
function renderLowerThirdSlotButtons(data) {
    if (!lowerThirdSlotButtons) return;
    lowerThirdSlotButtons.innerHTML = '';
    const tracks = (data && data.tracks && data.tracks.length)
        ? data.tracks
        : (lowerThirdLines ? parseLowerThirdLinesClient(lowerThirdLines.value) : []);
...
if (lowerThirdLines) {
    lowerThirdLines.addEventListener('input', () => {
        const base = lastLowerThirdServerData || { visible: false, activeIndex: 0 };
        const localTracks = parseLowerThirdLinesClient(lowerThirdLines.value);
...
socket.on('lowerThirdUpdate', (data) => {
    if (!data) return;
    lastLowerThirdServerData = data;
    if (lowerThirdLines && data.linesText !== undefined) {
        lowerThirdLines.value = data.linesText;
    }
    renderLowerThirdSlotButtons(data);
});
